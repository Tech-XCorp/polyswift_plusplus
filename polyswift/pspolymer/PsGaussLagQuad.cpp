...
// need here rather than in header (?)
using namespace std;

// Constructor
PsGaussLagQuad::PsGaussLagQuad(int m_order, double m_alpha) {

  alpha = m_alpha;   // power factor exponent
  order = m_order;   // number of points

  // Fixed for problem
  a=0;     // Default lower integral limit
  b = 0.0; // Scale parameter

  // Fixed for Gauss-Laguerre
  b     = 1.0;    // Default scale value
  beta  = 1.0;   // Not used for Gauss-Laguerre

  w = new double[order]; // weights
  x = new double[order]; // abscissas

  int kind = 5;  // General Gauss-Laguerre
  cgqf ( order, kind, alpha, beta, a, b, x, w );

  // Format results into std::vector-s
  for (size_t n=0; n<order; n++) {
    // std::cout << "x[" << n << "] = " << x[n]         << std::endl;
    // std::cout << "w[" << n << "] = " << w[n] << "\n" << std::endl;
    wVec.push_back(w[n]); // set output weight   vec
    xVec.push_back(x[n]); // set output abscissa vec
  }
}

// Destructor
PsGaussLagQuad::~PsGaussLagQuad() {
  delete [] w;
  delete [] x;
}
...
void PsGaussLagQuad::cdgqf(int nt, int kind, double alpha, double beta,
                           double t[], double wts[] )
...
void PsGaussLagQuad::cgqf ( int nt, int kind, double alpha, double beta, double a, double b,
  double t[], double wts[] )
...
double PsGaussLagQuad::class_matrix(int kind, int m, double alpha, double beta,
                                    double aj[], double bj[] )
//****************************************************************************
//
//  Purpose:
...
//
{
  ...
  if ( 500.0 * temp < r8_abs ( pow ( psGamma ( temp2 ), 2 ) - pi ) )
  {
    cout << "\n";
    cout << "CLASS_MATRIX - Fatal error!\n";
    cout << "  PsGamma function does not match machine parameters.\n";
    exit ( 1 );
  }
  ...
  {
    ab = alpha * 2.0;
    zemu = pow ( 2.0, ab + 1.0 ) * pow ( psGamma ( alpha + 1.0 ), 2 )
      / psGamma ( ab + 2.0 );
  ...
  else if ( kind == 4 )
  {
    ab = alpha + beta;
    abi = 2.0 + ab;
    zemu = pow ( 2.0, ab + 1.0 ) * psGamma ( alpha + 1.0 )
      * psGamma ( beta + 1.0 ) / psGamma ( abi );
    aj[0] = ( beta - alpha ) / abi;
  ...
  {
    ab = alpha + beta;
    zemu = psGamma ( alpha + 1.0 ) * psGamma ( - ( ab + 1.0 ) )
      / psGamma ( - beta );
    apone = alpha + 1.0;
    aba = ab * apone;
    aj[0] = - apone / ( ab + 2.0 );
...
void PsGaussLagQuad::imtqlx( int n, double d[], double e[], double z[] )
...
void PsGaussLagQuad::parchk( int kind, int m, double alpha, double beta )
...
double PsGaussLagQuad::r8_abs ( double x )
...
double PsGaussLagQuad::r8_epsilon ( )
...
double PsGaussLagQuad::r8_sign(double x)
//****************************************************************************80
//
//  Purpose:
//
...
void PsGaussLagQuad::scqf ( int nt, double t[], int mlt[], double wts[], int nwts,
                            int ndx[], double swts[], double st[], int kind,
...
void PsGaussLagQuad::sgqf(int nt, double aj[], double bj[], double zemu,
                          double t[], double wts[] )
